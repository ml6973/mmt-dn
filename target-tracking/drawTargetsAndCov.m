function drawTargetsAndCov(uu,V,F,colors,map,NOT)
NN=0;
red     = [1, 0, 0];
  green   = [0, 1, 0];
  blue    = [0, 0, 1];
  ...
  myColors=[red;green;blue;yellow;magenta];
target=zeros(3,NOT);
for i=1:NOT
...
t        = uu(NN+1);
NN=NN+1;
target_est=zeros(3,NOT);% estimated position of target
Pt_vec=zeros(25,NOT);
for i=1:NOT
    target_est(:,i)=uu(NN+30*(i-1)+1:NN+30*(i-1)+3);
    Pt_vec(:,i)=uu(NN+30*(i-1)+6:NN+30*(i-1)+30);
end
% time
size(Pt_vec);
% define persistent variables
persistent target_est_handle;
persistent cov_handle;
...
if t==0,
    figure(3), clf
    target_est_handle=zeros(1,NOT);
    cov_handle=zeros(1,NOT);
    target_handle=zeros(1,NOT);
    for i=1:NOT
       
        target_handle(i) = drawTarget(target(:,i), map.StreetWidth/30,red, [], 'normal');
        hold on
        
        target_est_handle(i) = drawTarget(target_est(:,i), map.StreetWidth/30,green, [], 'normal');
        hold on
        Pt=reshape(Pt_vec(:,i),5,5);
        PP=Pt(1:3,1:3);
         hold on
         cov_handle(i)=drawCovEllipse(target_est(:,i), PP, [],'normal');
    ...
    for i=1:NOT
        drawTarget(target(:,i), map.StreetWidth/30,red, target_handle(i));
        
        drawTarget(target_est(:,i), map.StreetWidth/30,green, target_est_handle(i));
        Pt=reshape(Pt_vec(:,i),5,5);
        PP=Pt(1:3,1:3);
        drawCovEllipse(target_est(:,i), PP,cov_handle(i));
    end
...
function handle=drawTarget(z, R,mycolor, handle, mode)
...
%red     = [1, 0, 0];
if isempty(handle),
    handle = fill(Y, X, mycolor, 'EraseMode', mode);
else
...
function handle=drawCovEllipse(xx, PP, handle, mode)

 pcov= make_feature_covariance_ellipses(xx,PP);

if isempty(handle),
    handle = plot(pcov(2,:), pcov(1,:), 'b', 'EraseMode', mode);
else
    set(handle,'XData',pcov(2,:),'YData',pcov(1,:));
end
