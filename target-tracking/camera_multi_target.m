...
%
function out = camera_multi_target(uu,P,NOT)
    ...
    NN = 0;
    for i=1:NOT
    tn(i)   = uu(6*(i-1)+1+NN); % target North position
    te(i)   = uu(6*(i-1)+2+NN); % target East position
    td(i)   = uu(6*(i-1)+3+NN); % target Down position
    end
    NN = NN + NOT*6;
    ...
    for i=1:NOT
    [eps_x, eps_y, eps_s] = cameraProjection(tn(i),te(i),td(i),pn,pe,pd,phi,theta,psi,az,el,P);
  
    out(3*(i-1)+1:3*(i-1)+3,1)=[eps_x;eps_y;eps_s];
    end
    
%     if t==0,  % initialize the plot
%         figure(1+id), clf, hold on
%     
%         % plot camera view 
