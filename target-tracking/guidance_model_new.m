function [sys,x0,str,ts,simStateCompliance] = guidance_model_new(t,x,u,flag,P,pn0,pe0,pd0,psi0,Va0)
...
switch flag,
  ...
  %%%%%%%%%%%%%%%%%%
  case 0,
    [sys,x0,str,ts,simStateCompliance]=mdlInitializeSizes(pn0,pe0,pd0,psi0,Va0);
...
%=============================================================================
%
function [sys,x0,str,ts,simStateCompliance]=mdlInitializeSizes(pn0,pe0,pd0,psi0,Va0)
...
%
x0  = [...
    pn0;...    % initial North position
    pe0;...    % initial East position
    psi0;...   % initial course
    0;...        % initial course rate
    -pd0;...   % initial altitude
    0;...        % initial climb rate
    Va0;...    % initial airspeed
    ];
...
function sys=mdlDerivatives(t,x,u,P)
 ...
 psi = chi - asin( (-P.wind_n*sin(chi)+P.wind_e*cos(chi))/Va );
  ...
  %pedot   = Vg*sin(chi);
  chiddot = -P.b_chidot*chidot + P.b_chi*(chi_c-chi);
 %% chiddot=SaturateGim(chiddot);
...
% end mdlTerminate
function out=SaturateGim(in)
psidotmax=9.8/20*tan(pi/6);
if (abs(in)>psidotmax)
    if(in>0)
        out=psidotmax;
    else
