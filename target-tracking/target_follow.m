...
%
function out = target_follow(in,P,h_d,Va_d,flag)
  ...
  switch flag,
      case 1, % follow straight line path specified by r and q
          ...
          k_path    = .001;        % proportional gain for path following
          
           % compute wrapped version of path angle
          chi_q = atan2(yt-pe,xt-pn);
      ...
      case 2, % follow orbit specified by c, rho, lam
          ...
          d = sqrt((pn-400)^2+(pe-100)^2); % distance from orbit center
          % compute wrapped version of angular position on orbit
          varphi = atan2(pe-yt,pn-xt);   
